...
class UserPreferencesRepository @Inject constructor(
    ...
    val isSecured: Flow<Boolean> = context.dataStore.data
        .catch { exception ->
            if (exception is IOException) {
                emit(emptyPreferences())
            } else {
        ...
        .map { preferences ->
            // No type safety.
            val pref = preferences[IS_SECURED] ?: false
            pref
        }
    ...
    suspend fun updateUserPin(pin: String) {
        context.dataStore.edit { preference ->
            preference[USER_PIN] = pin
        }
    }

    suspend fun updateSecureStatus(status : Boolean) {
        context.dataStore.edit { preference ->
            preference[IS_SECURED] = status
        }
    }
...
//        val IS_ON_BOARDING_DONE= booleanPreferencesKey("is_on_boarding_done")
        ...
        val USER_PIN = stringPreferencesKey("pc")
        val IS_SECURED = booleanPreferencesKey("is_secured")
    }
}
