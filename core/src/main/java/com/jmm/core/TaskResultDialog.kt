...
import com.jmm.core.databinding.FragmentTaskResultDialogBinding
...
class TaskResultDialog : DialogFragment() {
    ...
    override fun onViewCreated(view: View, savedInstanceState: Bundle?) {
        ...
        val code = params.getString(KEY_REDEEM_CODE)
        if (code.isNullOrEmpty()){
            binding.btnCopy.isVisible = false
            binding.tvRedeemCode.isVisible = false
        }else{
            binding.btnCopy.isVisible = true
            binding.tvRedeemCode.isVisible = true
            binding.tvRedeemCode.text = code

            binding.btnCopy.setOnClickListener {
                val myClipboard: ClipboardManager = requireActivity().getSystemService(CLIPBOARD_SERVICE) as ClipboardManager
                myClipboard.apply {
                    setPrimaryClip(ClipData.newPlainText("redeemCode", code))
                }
        ...
        binding.apply {
            when (params.getInt(KEY_WALLET_TYPE_ID)) {
                ...
                WalletType.OnlinePayment.id -> {
                    ivPaidUsing.setImageResource(R.drawable.ic_paytm)
    ...
    companion object{
        ...
        const val KEY_REDEEM_CODE="redeemCode"
